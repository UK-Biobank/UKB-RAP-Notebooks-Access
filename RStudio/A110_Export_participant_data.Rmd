...
get_dataset_id <- function() {
    ...
    record <- system("dx find data --class record | grep dataset", intern = TRUE)
    dataset <- list()
    dataset$id <- paste0(project, ":", stringr::str_extract(record, 'record.*(?=\\))'))
    dataset$prefix <- stringr::str_extract(record, 'app\\S*')
    return(dataset)
}
