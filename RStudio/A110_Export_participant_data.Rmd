...
get_dataset_id <- function() {
    ...
    record <- system("dx find data --type Dataset --delimiter ',' | sort -t ',' -k2,2r | head -n1 | awk -F ',' '{print $5}'", intern = TRUE)      # Selects the latest dataset for extraction when a project contains multiple datasets
    dataset <- list()
    dataset$id <- paste0(project, ":", record)
    dataset$prefix <- stringr::str_sub(system("dx find data --type Dataset --delimiter ',' | sort -t ',' -k2,2r | head -n1 | awk -F ',' '{print $4}'",         intern = TRUE), 2) # Selects the latest dataset for extraction when a project contains multiple datasets
    return(dataset)
}
